...
static az_span COMMAND_NAME_FAN_UP=AZ_SPAN_FROM_STR("FAN_TEMP_UP");
static az_span COMMAND_NAME_FAN_DOWN=AZ_SPAN_FROM_STR("FAN_TEMP_DOWN");
...
void azure_pnp_init() {
  ...
  // Check if there are any sensors attached
  ...
  // Try to get the address of the first sensor
  if (!sensors.getAddress(tempDeviceAddress, 0)) {
    Serial.println("Unable to find address for Device 0. Please check the wiring.");
    while (1);
  } else {
  ...
  // Set the resolution to 12 bit (optional)
...
int azure_pnp_handle_command_request(azure_iot_t* azure_iot, command_request_t command)
{
  ...
  else if (az_span_is_content_equal(command.command_name,COMMAND_NAME_FAN_UP))
  {
    FAN_TEMP++;
    LogInfo("Received FAN_TEMP UP", FAN_TEMP);
    Serial.println(FAN_TEMP);
    response_code = COMMAND_RESPONSE_CODE_ACCEPTED;
  }
  else if (az_span_is_content_equal(command.command_name,COMMAND_NAME_FAN_DOWN))
  {
    FAN_TEMP--;
    LogInfo("Received FAN_TEMP DOWN", FAN_TEMP);
    Serial.println(FAN_TEMP);
    response_code = COMMAND_RESPONSE_CODE_ACCEPTED;
  }
  else
    ...
    size_t* payload_buffer_length)
{
  ...
  // Turn on the relay if temperature is above 30°C
  if (temperatureC > FAN_TEMP) {
